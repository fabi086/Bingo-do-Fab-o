                ...
                {(isGameActive || drawnNumbers.length > 0) && (
                     <div className='text-center'>
                        <div className="mb-4">
                            ...
                            <p className="text-8xl font-black text-yellow-300 my-4 animate-pop-in">{drawnNumbers.length > 0 ? drawnNumbers[drawnNumbers.length - 1] : '-'}</p>
                        </div>
              ...
              <div>
                ...
                {cardViewMode === 'carousel' ? (
                     <div className="relative max-w-md mx-auto">
                          <div className={`${bingoWinner?.cardId === myCards[currentCardIndex]?.id ? 'ring-4 ring-yellow-400 animate-pulse' : ''} rounded-lg`}>
                    ...
                    <div className="space-y-6 max-w-md mx-auto">{myCards.map((card, index) => (<div key={card.id}><h3 className="text-center font-bold text-lg text-gray-300 mb-2">Cartela {index + 1}</h3><div className={`${bingoWinner?.cardId === card.id ? 'ring-4 ring-yellow-400 animate-pulse' : ''} rounded-lg`}><BingoCard cardData={card.cardData} drawnNumbers={new Set(drawnNumbers)} isAutoMarking={isAutoMarking} manualMarks={new Set(manualMarks[card.id] || ['LIVRE'])} onCellClick={(num) => handleCellClick(card.id, num)} /></div></div>))}</div>
                )}
              </div>
            )}
          </div>
            ...
            <InfoCard icon="👥" title="Jogadores na Sala">
                <ul className="space-y-2 max-h-48 overflow-y-auto">
                    {allPlayers.map(name => {
                        ...
                        return (
                            <li key={name} className={`flex items-center justify-between gap-3 p-1 rounded ${isWinner ? 'bg-yellow-400/20' : ''}`}>
                                <div className="flex items-center gap-3">
                                    <span className={`w-3 h-3 rounded-full ${isWinner ? 'bg-yellow-400' : 'bg-green-400'} animate-pulse`}></span>
